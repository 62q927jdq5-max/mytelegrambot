...
def webhook():
    ...
    if data and 'message' in data:
        ...
        # === ОТПРАВКА АДМИНУ (карточка + текст без шапки) ===
        card = (
            f"{t['card_title']}\n"
            f"─────────────────\n"
        ...
        # Текст сообщения — БЕЗ шапки "💬 Текст сообщения:"
        requests.post(
            f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
            json={
                "chat_id": ADMIN_CHAT_ID,
        ...
        # === ЕСЛИ ЭТО /start — ПОКАЗЫВАЕМ ПРИВЕТСТВИЕ ===
        if text == '/start':
            requests.post(
                f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
                json={
        ...
        # === ЛЮБОЕ ДРУГОЕ СООБЩЕНИЕ (НЕ КНОПКА, НЕ /start) ===
        else:
            # Просто показываем кнопки, без приветствия
            requests.post(
                f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",
                json={
                    "chat_id": chat_id,
